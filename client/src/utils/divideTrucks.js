...
const divideTrucks = async ({ addresses, numberOfTrucks }) => {
    ...
    for (let i = 0; i < addresses.length; i++) {
        ...
        for (let j = 0; j < addresses[i].items.length; j++) {
            summ += +addresses[i].items[j].shipment_count;
        }
        weights.push(summ);
    ...
    for (let i = 0; i < addresses.length; i++) {
        ...
        for (let j = i; j < addresses.length; j++) {
            ...
            const url = `https://api.distancematrix.ai/maps/api/distancematrix/json?origins=${addresses[i].lat},${addresses[i].lng}&destinations=${addresses[j].lat},${addresses[j].lng}&key=KWICdmSRFQF60bbuvwzwdoo80zP6Q`;

            // change to real url after getting new api key
